...
void initializeInterface(HWND hwnd)
{
   ...
   attempText = CreateWindowEx(WS_EX_LTRREADING, L"Edit", L"", WS_CHILD | WS_VISIBLE | SS_CENTER | SS_CENTERIMAGE, 348, 407, 68, 15, hwnd , NULL, NULL, NULL);

   // Establecer el límite máximo de caracteres
   SendMessage(attempText, EM_SETLIMITTEXT, NUM_DIGITS, 0);

   // Se crea el botón
   CreateWindowEx(WS_EX_LTRREADING, L"Button", L"Adivinar", WS_CHILD | WS_VISIBLE | WS_BORDER, 348, 425, 68, 30, hwnd, (HMENU) NEW_ATTEMPT, NULL, NULL);
}
